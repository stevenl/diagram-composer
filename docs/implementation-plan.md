...
1. Create the `modules/` directory with subfolders: `core/`, `adapter-api/`, `adapter-plantuml-c4/`, `ui/`, `intellij-plugin/`. Create top-level `tests/` and `docs/` alongside `modules/`. (Do not create `adapter-mermaid/` yet — see Milestone 11.)
2. Configure root `settings.gradle.kts` to `include(":modules:core")` etc. for each module, and root `build.gradle.kts` with Kotlin DSL, versions catalog, and module wiring (no logic yet — just modules that compile).
...
5. Add IntelliJ Platform SDK plugin setup to `intellij-plugin` (empty plugin.xml that builds).
6. Create doc stubs: `architecture.md`, `product.md`, `ui.md`, `adapters.md`, `engineering.md`, `development.md` (headings only, "TBD" bodies).
7. Add `.github/workflows/ci.yml`: triggers on `push` (any branch) and `pull_request`; runs `./gradlew build test` with Gradle dependency caching. This is the only gate for branches/PRs — no release side effects.
8. Decide and document a versioning strategy (e.g. a version field in `gradle.properties`, bumped as part of the PR that warrants a release, vs. fully automated semantic-release-style tagging) — flag as an assumption to confirm before task 9, since it drives how the release workflow determines the tag/version.
9. Add `.github/workflows/release.yml`: triggers on `push` to `main` only; runs the full build/test suite, builds the distributable plugin artifact (`intellij-plugin` packaging task), tags the commit with the version from task 8, and publishes a GitHub Release with the artifact attached and an auto-generated changelog from commit history. Does **not** publish to the JetBrains Marketplace — flag that as a separate future decision if/when we want public distribution.
10. Document the required branch protection rule in `development.md` (or `engineering.md`): `main` requires the `ci.yml` build to pass before merge. This is a GitHub repo setting, not something a workflow file enforces — note it as a manual one-time setup step, not a coding task.

**Definition of done:** `./gradlew build` succeeds across all modules; empty plugin can be built as a `.zip`/`.jar` artifact; a push to a feature branch and a PR both trigger `ci.yml` and it passes; merging to `main` triggers `release.yml` and produces a tagged GitHub Release with the built plugin artifact attached.
